...
# ════════════════════════════════════════════════════════
# ХРАНЕНИЕ ДАННЫХ через Streamlit query_params
# Данные сжимаются zlib + base64 и кладутся в ?d=...
# При обновлении страницы URL сохраняется → данные живут
# ════════════════════════════════════════════════════════
...
def save_state():
    ...
    try:
        st.query_params["d"] = encode_data(data)
    except Exception:
        pass

def load_state():
    """Загрузить портфель из URL."""
    try:
        ...
        if raw:
            ...
            if port:
                ...
                hist = data.get("h", {})
                st.session_state.history = {int(k): v for k, v in hist.items()} if hist else {}
                trd = data.get("t", {})
                st.session_state.trades = {int(k): v for k, v in trd.items()} if trd else {}
                st.session_state.next_id = data.get("id", 10)
                return True
    except Exception:
        pass
    return False
...
for k, v in {
    ...
    "history": {}, "portfolio_analysis": None, "dark_mode": True,
    "state_loaded": False, "trades": {},
}.items():
...
def analyze_stock(pos, mkt=None):
    ...
    lots_str = ", ".join("L"+str(i+1)+":"+str(l["shares"])+"@$"+str(l["price"])
                         for i,l in enumerate(pos["lots"]))
    avg_note = f"Averaged {ad}x, avg down {adp:.1f}%" if ad > 1 else "Single entry."
    mf = ""
    if mkt:
        mf = (f"\nREAL MARKET DATA:\n- Price: ${mkt['price']}\n"
              f"- Change: {mkt['change_pct']:+.2f}%\n"
              f"- 52w H/L: ${mkt['high_52w']}/${mkt['low_52w']}\n"
              f"- P&L: {pl_pct:+.1f}%")
    schema = ('{"recommendation":"HOLD","risk":"HIGH","confidence":55,"delistingRisk":false,'
    ...
    prompt = (f"Prop trader. Analyze position.\n"
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f'💾 Данные сохраняются в URL</div>', unsafe_allow_html=True)
    ...
    for pos in st.session_state.portfolio:
        ...
        for i, lot in enumerate(pos["lots"]):
            ...
            if c2.button("✕", key=f"rm_{pos['id']}_{i}"):
                pos["lots"].pop(i); save_state(); st.rerun()
        ...
        if pos["lots"]:
            with st.expander(f"💰 Продать {pos['ticker']}"):
                ...
                if sell_sh > 0 and sell_pr > 0:
                    ...
                    pc = "#a8ff3e" if trade_pnl >= 0 else "#ff3ea8"
                    st.markdown(
                        f'<div style="text-align:center;font-size:13px">'
                        f'P&L: <span style="color:{pc};font-weight:700">'
                        f'{"+" if trade_pnl>=0 else ""}${trade_pnl:.2f}</span></div>',
                        unsafe_allow_html=True)
                if st.button("✅ Продать", key=f"sell_{pos['id']}", use_container_width=True):
                    if sell_sh > 0 and sell_pr > 0 and sell_sh <= max_sh:
                        ...
                        entry = {"date": datetime.now().strftime("%d.%m.%Y %H:%M"),
                                 "ticker": pos["ticker"], "shares": sell_sh,
                                 "sell_price": sell_pr, "avg_cost": round(avg_now,4),
                                 "pnl": round(trade_pnl,2), "pnl_pct": round(trade_pnl_pct,2)}
                        pid = pos["id"]
                        if pid not in st.session_state.trades:
                            st.session_state.trades[pid] = []
                        st.session_state.trades[pid].insert(0, entry)
                        # FIFO списание лотов
                        remaining = sell_sh; new_lots = []
                        for lot in pos["lots"]:
                            if remaining <= 0:
                                new_lots.append(lot)
                                ...
                                new_lots.append({"shares": lot["shares"]-remaining, "price": lot["price"]})
                                remaining = 0
                        pos["lots"] = new_lots
                        if not pos["lots"]:
                            st.session_state.portfolio = [p for p in st.session_state.portfolio if p["id"] != pid]
                ...
                with st.spinner(f"Анализирую {pos['ticker']}..."):
                    try:
                        ...
                        entry  = {"date": datetime.now().strftime("%d.%m %H:%M"),
                                  ...
                                  "pl_pct": (result["currentPrice"]-avg_p)/avg_p*100
                                            if result.get("currentPrice") and avg_p > 0 else None}
        ...
        if b2.button("🗑", key=f"del_{pos['id']}", use_container_width=True):
            ...
            st.session_state.trades.pop(pos["id"], None)
...
if not sel_res:
    ...
    # ── БАЛАНС ───────────────────────────────────────────
    realized   = sum(t["pnl"] for ts in st.session_state.trades.values() for t in ts)
    unrealized = 0.0
    invested   = 0.0
    for p in st.session_state.portfolio:
        if not p["lots"]: continue
        avg  = avg_price(p["lots"]); cost = total_cost(p["lots"]); invested += cost
        mkt  = get_market_data(p["ticker"]) if YFINANCE_OK else None
        r2   = st.session_state.results.get(p["id"])
        lp   = mkt["price"] if mkt else (r2.get("currentPrice") if r2 else None)
        if lp and avg > 0:
            unrealized += (lp - avg) * total_shares(p["lots"])

    total = realized + unrealized
    tc = "#a8ff3e" if total >= 0 else "#ff3ea8"
    rc = "#a8ff3e" if realized >= 0 else "#ff3ea8"
    uc = "#a8ff3e" if unrealized >= 0 else "#ff3ea8"

    st.markdown(f'<h3 style="color:{TEXT}">💼 Баланс</h3>', unsafe_allow_html=True)
    b1,b2,b3,b4 = st.columns(4)
    for col, val, lbl, color in [
        (b1, total,      "Итог (реал.+нереал.)",   tc),
        (b2, realized,   "Реализовано (продажи)",  rc),
        (b3, unrealized, "Нереализовано (сейчас)", uc),
        (b4, invested,   "Вложено сейчас",         TEXT),
    ]:
        col.markdown(
            f'<div style="background:{BG2};border:1px solid {color}33;border-radius:8px;padding:14px;text-align:center">'
            f'<div class="label">{lbl}</div>'
            f'<div style="color:{color};font-size:20px;font-weight:700">'
            f'{"+" if val > 0 else ""}${val:.2f}</div></div>', unsafe_allow_html=True)

    # ── история продаж ─────────────────────────────────────
    all_trades = sorted(
        [t for ts in st.session_state.trades.values() for t in ts],
    ...
    if all_trades:
        ...
        for t in all_trades:
            ...
            st.markdown(
                ...
                f'({"+" if t["pnl_pct"]>=0 else ""}{t["pnl_pct"]:.1f}%)</span></div>',
                unsafe_allow_html=True)
